...
export const buildFileKeys = (granularity: 'hourly' | 'daily', fileNames: readonly string[]): readonly string[] => {
	const fileKeys: readonly string[] = fileNames.map(
		(fileName) => `${ARENA_STATS_KEY_PREFIX}/decks/${granularity}/${fileName}.gz.json`,
	);
	return fileKeys;
};

export const getFileKeysToLoad = (timePeriod: TimePeriod, patchInfo: PatchInfo): readonly string[] => {
	// We want to load:
	...
	// for the patch day
	const currentDayHourlyKeys = getHourlyKeysForCurrentDay();
	// console.debug('currentDayHourlyKeys', currentDayHourlyKeys);
	const previousDaysDailyKeys = getDailyKeysForPreviousDays(timePeriod, patchInfo);
	// console.debug('previousDaysDailyKeys', previousDaysDailyKeys);
	const patchDayHourlyKeys = getHourlyKeysForPatchDay(timePeriod, patchInfo);
	// console.debug('patchDayHourlyKeys', patchDayHourlyKeys);
	return [...currentDayHourlyKeys, ...previousDaysDailyKeys, ...patchDayHourlyKeys];
};

const getHourlyKeysForCurrentDay = (): readonly string[] => {
	...
	for (let i = 0; i < now.getHours() + 1; i++) {
		...
		// The date in the format YYYY-MM-ddTHH:mm:ss.sssZ
		const dateStr = date.toISOString();
		keys.push(`${ARENA_STATS_KEY_PREFIX}/decks/hourly/${dateStr}.gz.json`);
	}
	return keys;
};

const getDailyKeysForPreviousDays = (timePeriod: TimePeriod, patchInfo: PatchInfo): readonly string[] => {
	const firstDate = computeStartDate(timePeriod, patchInfo);
	const keys: string[] = [];
	while (firstDate < new Date()) {
		firstDate.setDate(firstDate.getDate() + 1);
		const dateStr = firstDate.toISOString();
		keys.push(`${ARENA_STATS_KEY_PREFIX}/decks/daily/${dateStr}.gz.json`);
	}
	return keys;
};
...
const getHourlyKeysForPatchDay = (timePeriod: TimePeriod, patchInfo: PatchInfo): readonly string[] => {
