...
import { buildHighWinRuns } from './high-win-runs';
import { saveHighWinRuns } from './persist-data';
...
export default async (event, context: Context): Promise<any> => {
	...
	const rows: readonly InternalArenaRunRow[] = await loadRows(processStartDate, processEndDate);
	const highWinRuns: readonly ArenaRunInfo[] = await buildHighWinRuns(rows);

	await saveHighWinRuns(highWinRuns, processStartDate, s3);
