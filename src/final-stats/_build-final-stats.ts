...
import { filterRuns } from './filter-data';
import { loadDailyHighWinRunsFromS3 } from './s3-loader';
import { saveHighWinRuns } from './s3-saver';
...
export default async (event, context: Context): Promise<any> => {
	...
	const dailyData: readonly HighWinRunsInfo[] = await loadDailyHighWinRunsFromS3(timePeriod, patchInfo);
	const filteredData: readonly ArenaRunInfo[] = filterRuns(dailyData);
	await saveHighWinRuns(filteredData, timePeriod);
